...
class ipsPayment {
    ...
    public static function ToSubmit($payment){
   		$submitUrl = 'https://pay.ips.com.cn/ipayment.aspx?';
	...
	//商户返回地址
       $url_h = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') ? 'https://' : 'http://';
       $url_h .= str_ireplace('localhost', '127.0.0.1', $_SERVER['HTTP_HOST']) . $_SERVER['SCRIPT_NAME'];
       $url_h = str_ireplace('orderpay', 'OrderReturn', $url_h);
	   $SignMD5 = strtolower(md5("billno".$payment['trade_no']."currencytype".$Currency_Type."amount". $Amount ."date". $Date . "orderencodetype".$orderencodetype.$Mer_key));
